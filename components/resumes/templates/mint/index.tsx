...
/**
 * Template Mint Green de CV (inspiré de l'image Aiden Williams)
 * Caractéristiques:
 * - Design clean et moderne
 * - Palette verte menthe (#4ECDC4)
 * - Layout une colonne avec sections bien définies
 * - Photo circulaire avec fond coloré
 * - Typographie sans-serif claire
 * - Icônes et capsules colorées pour les sections
 */
export const MintGreenTemplate: React.FC<ResumeTemplateProps> = ({
  resume,
  isEditable = false,
  onEditSection,
}) => {
    ...
    >
      ...
      <header className="p-8 pb-6" onClick={() => handleEditSection('personalInfo')}>
        <div className="flex items-start space-x-6">
          ...
          <div className="flex-1">
            <h1 className="text-4xl font-bold tracking-wide mb-2" style={{ color: theme.text }}>
      ...
      <div className="px-8 space-y-8">
                    ...
                    <ul className="space-y-1">
                      {skillsInCategory.map(skill => (
                        <li key={skill.id} className="flex items-center text-sm">
                          <span
                            className="mr-2 w-1 h-1 rounded-full"
                            style={{ backgroundColor: theme.primary }}
                          />
                          {skill.name}
                          {skill.level && (
        ...
        {achievements && achievements.length > 0 && (
          <section onClick={() => handleEditSection('achievements')} className="pb-8">
            <div className="flex items-center mb-4">
              <div
                className="w-8 h-8 rounded-full flex items-center justify-center mr-3"
                style={{ backgroundColor: theme.primary }}
